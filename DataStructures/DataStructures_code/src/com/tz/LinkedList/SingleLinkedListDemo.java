...
//        singleLinkedList.add(hero4);
    ...
    public static void reverseList(HeroNode head) {
        ...
        // 理解思路：1.将cur指向当前链表head的下一个节点（其实就是第一个有效节点）
        //         2.用next保存cur的下一个节点
        //         3.首先将cur.next指向null
        //         4.然后将虚拟节点将当前cur链接起来
        //         5.cur后移赋值为第二个有效节点
        //         6.当第二次循环时首先将next赋值为cur的下一个节点（其实就是第三个）
        //         7.然后将当前结点的next指针指向虚拟节点的下一个（其实也就是第一个有效节点）
        //         8.然后将虚拟节点的next指向当前结点（其实也就是第二个有效节点）
        //         9.cur后移
        //         10.依次循环遍历（每次都将新节点加在虚拟节点的的下一个，最后将旧head的next指向虚拟节点的next达到反转效果）
